...
export default function ManageUsers() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      await createUser({ username: username.trim(), password, role, full_name: fullName.trim() });
      setUsername("");
      setPassword("");
      setFullName("");
      await refresh();
    ...
    <div>
      ...
      <Card className="mb-6 border-amber-200 bg-amber-50">
        <CardContent className="text-xs text-amber-800 p-4 leading-relaxed">
          ...
          reset when it restarts. Passwords are not hashed.
        </CardContent>
      </Card>
          ...
          <CardContent>
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <Input value={username} onChange={(e) => { setUsername(e.target.value); setError(""); }} placeholder="e.g. jdoe" className="mt-1.5" />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Full name</label>
                <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Jane Doe" className="mt-1.5" />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Password</label>
                <Input type="password" value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }} placeholder="••••••••" className="mt-1.5" />
              </div>
              <div>
              ...
              <div className="space-y-3">
                {users.map(u => (
                  <div key={u.id} className="border border-gray-100 rounded-lg p-4 flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${u.role === "admin" ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600"}`}>
                      {u.role === "admin" ? <Shield size={18} /> : <UserCircle size={18} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-[var(--bot-navy)]">{u.full_name || u.username}</div>
                      <div className="text-xs text-gray-500 mt-0.5">@{u.username}</div>
                    </div>
                    <Badge className={`capitalize border font-bold ${u.role === "admin" ? "bg-red-50 text-red-700 border-red-200" : "bg-blue-50 text-blue-700 border-blue-200"}`}>
                      {u.role}
                    </Badge>
                    <button onClick={() => handleDelete(u.id)} className="text-gray-400 hover:text-red-500 transition p-1" title="Delete user">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
